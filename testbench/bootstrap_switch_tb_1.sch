...
"}
C {devices/code_shown.sym} -560 -500 0 0 {name=NGSPICE
only_toplevel=true
value="
...
.control
  ...
  let v_stop  = 1.20
  let v_step  = 0.02
  shell rm -f boot_diag.txt
  echo Vin Vsampled error_mV >> boot_diag.txt
  let vin_val = v_start
  dowhile vin_val <= v_stop + 1e-9
    reset
    alter V2 = $&vin_val
    tran 50p 480n
    let last = length(time) - 1
    let vsamp = v(vo)[last]
    let err = (vsamp - vin_val) * 1e3
    echo $&vin_val $&vsamp $&err
    echo $&vin_val $&vsamp $&err >> boot_diag.txt
    let vin_val = vin_val + v_step
  end
  echo Done
.endc
.end
" }
C {vsource.sym} 60 -370 0 0 {name=V1 value="PULSE(0 1.2 0 50p 50p 500n 750n)"}
C {gnd.sym} 60 -320 0 1 {name=l2 lab=GND}
C {lab_pin.sym} 60 -430 0 0 {name=p1 sig_type=std_logic lab=clk}
C {vsource.sym} 60 -510 0 0 {name=V2 value=0.2}
